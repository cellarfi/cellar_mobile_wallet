...
export async function createAndBuyTokenViaPumpfun({
  ...
  const pumpFunPlatformFeeRecipient = new PublicKey(
    ENV.SOLANA_COMMISSION_WALLET!
  )
  // This is the recipient for our additional 0.5% platform commission.
  const ourPlatformCommissionWallet = new PublicKey(
    ENV.SOLANA_COMMISSION_WALLET!
  )
  ...
  try {
    ...
    const uploadEndpoint = `${ENV.SERVER_URL}/api/pumpfun/uploadMetadata`
    ...
    formData.append('createdOn', 'https://www.cellar.so')
    ...
    if (solAmountForPumpFunBuyLamports > 0 && solAmount > 0) {
      ...
      buyIx = await pumpSdk.buyInstructions({
        ...
        bondingCurveAccountInfo: {
          data: Buffer.alloc(0), // or the actual buffer if you have it
          executable: false,
          lamports: 0,
          owner: creatorPubkey, // or the correct owner PublicKey
          rentEpoch: 0,
        } as unknown as AccountInfo<Buffer<ArrayBufferLike>>,
        bondingCurve,
        mint: mint.publicKey,
        amount: buyTokenAmount,
        associatedUserAccountInfo: null,
        user: creatorPubkey,
        solAmount: solAmountForBuyBN,
        slippage: 1,
      })
