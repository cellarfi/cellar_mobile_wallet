...
const TokenCallCard = ({ token_meta }: { token_meta: Posts["token_meta"] }) => {
  const isUpcoming =
    ...
    <View className="flex-row items-center mb-2 relative">
      {post.user.profile_picture_url ? (
        <Image
          ...
          className="w-10 h-10 rounded-full bg-dark-300 mr-2"
        />
      ) : (
      ...
      {showMenu && (
        <>
          ...
          />
          <View
            className="absolute z-40 right-0 top-10 bg-dark-200 rounded-xl shadow-lg border border-dark-300 min-w-[160px]"
            style={{ elevation: 8 }}
          >
            <TouchableOpacity
      ...
      <View>
        <TouchableOpacity
          onPress={() => {
            router.push({
              pathname: "/(modals)/send",
              params: { recipient: post.user.wallets[0]?.address, currentMemo: post.user.display_name + " Tip", },
            });
          }}
