...
const MediaAttachment: React.FC<MediaAttachmentProps> = ({
  ...
  const handleMediaPick = useCallback(async () => {
    if (maxAttachments && mediaItems.length >= maxAttachments) {
      Alert.alert(
    ...
    try {
      const result = await ImagePicker.launchImageLibraryAsync({
        ...
        selectionLimit: maxAttachments ? maxAttachments - mediaItems.length : 0,
        quality: 0.8,
      });
      ...
      for (const asset of result.assets) {
        ...
        // Check file size
        if (maxFileSize && asset.fileSize && asset.fileSize > maxFileSize) {
          Alert.alert(
            'File too large',
      ...
      return (
        <View className="mt-2">
          ...
          <Text className="text-xs text-gray-500 mt-1 ml-1">
            {maxAttachments ? maxAttachments : 0} files max •{' '}
            {maxFileSize ? maxFileSize / 1024 / 1024 : 0}MB per file
          </Text>
        </View>
      );
    }
    ...
    return (
      <View className="mt-2 mb-3">
        ...
        >
          ...
          {mediaItems.length < (maxAttachments || 0) && (
            <TouchableOpacity
              onPress={handleMediaPick}
        ...
        <Text className="text-xs text-gray-400 mt-1">
          {mediaItems.length}/{maxAttachments || 0} files •{' '}
          {maxFileSize ? maxFileSize / 1024 / 1024 : 0}MB max per file
        </Text>
      </View>
    );
  };
