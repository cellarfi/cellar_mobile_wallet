...
const DonationCard = React.memo(
  ({ funding_meta, postId }: DonationCardProps) => {
    ...
    const handleDonatePress = () => {
      // Allow donations for both ACTIVE and COMPLETED campaigns
      if (
        (funding_meta.status === 'ACTIVE' ||
          funding_meta.status === 'COMPLETED') &&
        funding_meta.wallet_address
      ) {
        router.push({
          pathname: '/(modals)/donate' as any,
          params: {
            postId: postId || '',
            walletAddress: funding_meta.wallet_address,
            campaignTitle: 'Donation Campaign',
            currentAmount: funding_meta.current_amount || '0',
            targetAmount: funding_meta.target_amount || '0',
          },
        })
      }
    }
