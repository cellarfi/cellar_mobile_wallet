      ...
      send(rpcRequestOrMethod, callbackOrParams) {
        if (typeof rpcRequestOrMethod !== 'string') {
          return this.sendAsync(
            rpcRequestOrMethod,
            callbackOrParams as SendCallback
          )
        }

        return this.request({
          method: rpcRequestOrMethod,
          params: callbackOrParams,
        })
      },
      sendAsync(rpcRequest, callback) {
        const id =
          'id' in rpcRequest && typeof rpcRequest.id === 'number'
            ? rpcRequest.id
            : undefined
    ...
    try {
      ...
      if ('network' in responseOrEvent) {
        const provider: Provider =
          responseOrEvent.network === 'evm' ? evmProvider : solanaProvider
...
console.log('getInjectedScriptString', injectProviders.toString())
